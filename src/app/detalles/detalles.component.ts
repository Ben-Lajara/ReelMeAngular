...
export class DetallesComponent implements OnInit, AfterViewInit {
  ...
  vista = false;
  puntuacionMedia = '';
  frecuenciasDiccionario = {0.5: 0, 1: 0, 1.5: 0, 2: 0, 2.5: 0, 3: 0, 3.5: 0, 4: 0, 4.5: 0, 5: 0};
  frecuencias = new Map([
  ...
  ngOnInit(): void {
    this.route.params.subscribe((params) => {
      ...
      this.getReviewsPelicula().subscribe((res: any) => {
        ...
        this.getResenasSeguidos(this.currentUsername, this.id).subscribe((res: any) => {
  ...
  getActividad(usuario: string, id_pelicula: string) {
    ...
    return this.http.get(`http://localhost:8080/api/review?usuario=${usuario}&idPelicula=${id_pelicula}`).pipe(
      tap(res => {
        this.resena = res;
        if(this.resena) {
          this.vista=true;
  ...
  getResenasSeguidos(nombre: string, idPelicula: string){
    return this.http.get(`http://localhost:8080/resenasSeguidosPor/${nombre}/${idPelicula}`);
            ...
            tooltip:{
              ...
              callbacks: {
                label: function(context){
                  let total = context.dataset.data.reduce((a, b) => (a as number) + (b as number), 0) as number;
